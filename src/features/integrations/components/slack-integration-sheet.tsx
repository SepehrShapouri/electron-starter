...
export function SlackIntegrationSheet({
  accounts,
}: {
  ...
  const connectMutation = useMutation({
    mutationFn: () => {
      ...
      return integrationsApi
        .connectComposioToolkit('slack', redirectUrl)
  ...
  return (
    <div className="space-y-4">
      {accounts.length > 0 ? (
        <div>
          <p className="mb-2 text-sm font-medium">Connected workspaces</p>
          <ul className="space-y-1.5" aria-label="Connected Slack workspaces">
            {accounts.map(account => {
              const label = getAccountLabel(account, 'Slack workspace');
