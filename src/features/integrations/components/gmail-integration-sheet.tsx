...
export function GmailIntegrationSheet({
  accounts,
}: {
  ...
  const connectMutation = useMutation({
    mutationFn: () => {
      ...
      return integrationsApi
        .connectComposioToolkit('gmail', redirectUrl)
    ...
    <div className="space-y-4">
      {accounts.length > 0 ? (
        <div>
          ...
          <ul className="space-y-1.5" aria-label="Connected Gmail accounts">
            {accounts.map(account => {
              const label = getAccountLabel(account, 'Gmail account');
