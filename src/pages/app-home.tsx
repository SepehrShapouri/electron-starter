...
export default function AppHome() {
  ...
  const isEmptyConversation = useMemo(
    () => !hasConversationItems(messages, toolEntries),
    [messages, toolEntries]
  );

  useEffect(() => {
    if (!resolvedChatConfig.gatewayUrl || connected) {
      return;
    }
    if (status === 'idle') {
      connect();
  ...
  const promptComposer = (
    <ChatComposer
      gatewayUrl={resolvedChatConfig.gatewayUrl}
      connected={connected}
  ...
  return (
    <div className="relative mx-auto flex h-full w-full max-w-[700px] flex-col px-2">
      <div className="min-h-0 flex flex-1 flex-col overflow-hidden ">
        {isEmptyConversation ? (
            ...
            <div className="w-full flex flex-col">
              <div className="flex flex-col gap-6 items-start">
                ...
                <div className="flex flex-col gap-2">
                  {firstName ? <p className="text-lg">Hi, {firstName}!</p> : null}
          ...
          <ChatConversation messages={messages} toolEntries={toolEntries} />
        )}
      </div>
      {!isEmptyConversation ? (
        <>
          <QueuePill queue={queue} onRemove={removeFromQueue} />
          <div className="shrink-0 pb-8 backdrop-blur-xl">
            {promptComposer}
          </div>
        </>
      ) : null}
