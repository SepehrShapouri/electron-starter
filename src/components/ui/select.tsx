  ...
  "focus-visible:border-ring focus-visible:ring-ring/50 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive dark:aria-invalid:border-destructive/50 data-placeholder:text-muted-foreground gap-1.5 border py-2 pr-2 transition-colors select-none focus-visible:ring-3 aria-invalid:ring-3 [&_svg:not([class*='size-'])]:size-4 flex w-fit items-center justify-between whitespace-nowrap outline-none disabled:cursor-not-allowed disabled:opacity-50 *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-1.5 [&_svg]:pointer-events-none [&_svg]:shrink-0",
  {
    variants: {
      ...
      size: {
        ...
        xl: "h-10 rounded-lg px-3 text-sm",
        xxl: "h-12 rounded-xl px-3.5 text-base [&_svg:not([class*='size-'])]:size-5",
      },
    },
    defaultVariants: {
      variant: "default",
    ...
    <SelectPrimitive.Portal>
      ...
      >
        <SelectPrimitive.Popup
          ...
          className={cn("bg-popover text-popover-foreground data-open:animate-in data-closed:animate-out data-closed:fade-out-0 data-open:fade-in-0 data-closed:zoom-out-95 data-open:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 ring-foreground/10 min-w-36 overflow-hidden rounded-lg shadow-fancy ring-1 duration-100 data-[side=inline-start]:slide-in-from-right-2 data-[side=inline-end]:slide-in-from-left-2 relative isolate z-50 max-h-(--available-height) w-(--anchor-width) origin-(--transform-origin) data-[align-trigger=true]:animate-none", className )}
          {...props}
        >
          <SelectScrollUpButton />
          <SelectPrimitive.List
            data-slot="select-list"
            className="no-scrollbar max-h-[min(calc(--spacing(72)---spacing(9)),calc(var(--available-height)---spacing(9)))] overflow-y-auto overscroll-contain scroll-py-1 p-1"
          >
            {children}
          </SelectPrimitive.List>
...
function SelectScrollUpButton({
  ...
  return (
    <SelectPrimitive.ScrollUpArrow
      ...
      {...props}
    >
      <AppIcon name="IconChevronDownMedium" className="rotate-180" />
    </SelectPrimitive.ScrollUpArrow>
  )
}
...
function SelectScrollDownButton({
  ...
  return (
    <SelectPrimitive.ScrollDownArrow
      ...
      {...props}
    >
      <AppIcon name="IconChevronDownMedium" />
    </SelectPrimitive.ScrollDownArrow>
  )
}
